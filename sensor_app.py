...
def main():
    ...
    try:
        ...
        print("\n3. 停止视觉跟随")
        bot.stop_application()  # 停止应用并停止底盘运动
        time.sleep(1)

        # 使用自定义HSV颜色范围的视觉跟随
        # HSV颜色空间比RGB更适合颜色检测，对光照变化不敏感
        print("\n4. 自定义HSV颜色跟随示例")
        print("使用自定义橙色 HSV范围")
        # HSV范围定义：H(色调) S(饱和度) V(亮度)
        hsv_lower = (10, 100, 100)  # 橙色下界：H=10(偏红的橙), S=100, V=100
        hsv_upper = (20, 255, 255)  # 橙色上界：H=20(偏黄的橙), S=255, V=255
        bot.start_visual_follow_custom(hsv_lower, hsv_upper, control_enabled=True)
        time.sleep(5)
        bot.stop_application()
        time.sleep(1)

        # 仅检测模式：不自动控制底盘，仅提供检测结果
        # 适合需要自定义控制逻辑的场景
        print("\n5. 视觉检测模式 (不控制底盘)")
