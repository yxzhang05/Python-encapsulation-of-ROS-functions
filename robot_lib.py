...
class Robot:
        ...
        for process, name in processes:
            if process:
                try:
                    ...
                    process.wait(timeout=5) # 等待最长5秒
                except subprocess.TimeoutExpired:
                    process.kill() # 如果关不掉，强制杀进程
    ...
    def move_distance(self, distance, speed, lateral_distance=0.0, lateral_speed=0.0, timeout=30.0):
        """
        闭环控制小车前进或后退指定距离
        :param distance: float, 移动距离(m), 负数代表后退
        :param speed: float, 移动速度(m/s), 必须为正数
        ...
        try:
            ...
            while (time.time() - start_time) < timeout:
                ...
                # 判断是否到达 (2cm误差)
                if remaining_x < 0.02 and remaining_y < 0.02:
                    break
    ...
    def start_visual_follow(self, color, control_enabled=True):
        ...
        :return: bool, 成功返回True
        注意: 使用 get_visual_target_info() 获取目标信息
        """
    ...
    def get_lidar_distance(self, angle):
        ...
        try:
            ...
            output = subprocess.check_output(cmd, timeout=2.0).decode("utf-8")
            # TODO: 解析LaserScan消息 (根据angle_min/max/increment计算索引)
            distance = 0.0
            return distance
        except Exception as e:
